...
- [x] Свободное назначение — ~~`cycle_slot_weapon()` крутит оружие в слоте по кругу (клавиша `Tab`)~~
      заменено на `set_slot_weapon()` + drag-and-drop из каталога в экране инвентаря (см. ниже,
      `feature/movement-by-cursor`) — `Tab` убран
...
- [ ] Смена найденного оружия в бою уже работает «из коробки» (слоты 1-8 + экран инвентаря
  `I` → перетащить оружие из каталога на слот, `PlayerCombat.set_slot_weapon`) — находка через
  будущий `WeaponPickup` лишь добавляет новый `weapon_id` в оборот, архитектурных изменений в
  `PlayerCombat` не требует
- [ ] Где монтировать модуль на оружие — экран инвентаря (`InventoryScreen`) уже есть (слоты +
  каталог), но нужен новый UI-блок конкретно под модули (пока показывает только weapon_id)
...
**Каталог оружия + все 8 слотов видны сразу** (`feature/movement-by-cursor`, по запросу
пользователя — убрать `Tab`, показать игроку весь потенциал прокачки слотов):
- [x] `Tab` (`cycle_slot_weapon`) убран — не осталось клавиатурного способа сменить тип
      оружия в слоте, только инвентарь
- [x] `InventoryScreen` всегда рисует все `MAX_WEAPON_SLOTS` (8) слотов, а не только уже
      разлоченные — locked-слоты видны, но неинтерактивны и вдвое прозрачнее
      (`InventoryScreen.LOCKED_ALPHA`), чтобы игрок видел, сколько слотов ещё открывать
- [x] Ряд-каталог всего `WEAPON_CATALOG` под слотами — drag-and-drop иконки оружия на
      разлоченный слот вызывает `PlayerCombat.set_slot_weapon(index, weapon_id)`
- [ ] **На будущее (по словам пользователя):** сейчас в каталоге доступны сразу все виды
      оружия, включая огнестрел. Когда появится дерево прокачки/условия открытия оружия,
      каталог в инвентаре должен показывать недоступное оружие тем же приёмом, что и
      locked-слоты (видно, но неактивно/затемнено) — сам `InventoryScreen` для этого менять
      не придётся, менять нужно будет только источник "доступно ли игроку это оружие"
...
- [x] Обновить `README.md`/`README_RU.md` — упрощены под игрока (убраны технические разделы:
      архитектура, GitFlow, версионирование, разбор CI/CD воркфлоу), актуализировано управление
      (WASD относительно прицела, оружие 1-8, R/ПКМ, инвентарь `I`)
- [x] `AGENTS.md` (устаревшее описание ECS/WorldManager) удалён — технические разделы из README
      перенесены и объединены с `CLAUDE.md`, который теперь единственный агентский файл
